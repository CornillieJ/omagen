...
func writeHyprlandWithAnimationsAndShell(themeDir string, p Palette, borderStyle string, borderSize int, shape, spacing, depth, active, inactive string, borderSpeed int, animations session.AnimationsStyle, windowOpacity *int, shellPreset string, barSpecs ...*bar.BarSpec) error {
	...
	case "spin":
		if accents := p.ActiveAccents(); len(accents) >= 2 {
			...
			// entirely.
			activeBorder = evenBandGradientLua(hyprColors(accents), 0)
		} else {
			// A two-stop accent/blue gradient can be nearly monochromatic in
			// extracted palettes. Add a contrasting magenta stop.
			activeBorder = evenBandGradientLua(hyprColors([]string{p.Accent, p.Blue, p.Magenta}), 0)
		}
	case "dual":
		...
		// renders sensibly for palettes that never set a second accent.
		accents := p.ActiveAccents()
		if len(accents) < 2 {
			accents = append(accents, accent2Or(p, p.Magenta))
		}
...
// accent2Or returns the palette's user-set second accent when it is a valid
// hex color, otherwise the provided fallback (kept as a raw hex string so
// callers can pass it straight to hyprColor).
func accent2Or(p Palette, fallback string) string {
	if validHex(p.Accent2) {
		return p.Accent2
	}
	return fallback
}

// hyprlandMaxGradientStops is a hard limit in Hyprland's border shader
// (gradient.glsl declares `vec4 gradient[10]`), not a stylistic choice.
// Passing more stops than this either gets silently truncated or reads
// undefined array slots, so no gradient builder here may exceed it.
const hyprlandMaxGradientStops = 10

// hyprColors converts a list of raw hex colors to Hyprland's rgb(...) form.
...
// evenBandGradientLua builds a Hyprland gradient across 2-5 colors, each
// getting an even share of the 0-1 gradient axis, capped at
// hyprlandMaxGradientStops total stops.
//
// This deliberately does NOT palindrome the colors (c1, c2, ..., c1): the
// border shader (gradient.glsl) projects the gradient across the window's
// (x, y) bounding box, not around the border's traced perimeter length, so
// the color at position 0 and the color at position 1 each end up covering
// a whole side of the rectangle (e.g. the left edge and the right edge, for
// a horizontal angle) rather than a proportional slice of the perimeter. A
// palindrome puts the same color at both ends, so that one color covers two
// full sides of the border while every other color is squeezed into a thin
// band on the remaining sides -- confirmed against a live report of a
// two-accent gradient rendering as ~90% one accent, ~10% the other. Using
// each color once, in order, gives every accent its own side/region instead.
//
// Hyprland only supports evenly spaced stops, with no explicit per-stop
// percentage (see hyprwm/Hyprland#2806, an open/unimplemented feature
// request), so repeating each color a few times still approximates a
// harder-edged band at each boundary, budgeted to the 10-stop limit.
