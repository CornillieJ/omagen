...
func writeHyprlandWithAnimationsAndShell(themeDir string, p Palette, borderStyle string, borderSize int, shape, spacing, depth, active, inactive string, borderSpeed int, animations session.AnimationsStyle, windowOpacity *int, shellPreset string, barSpecs ...*bar.BarSpec) error {
	...
	case "dual":
		...
		hyprAccents := make([]string, len(accents))
		for i, hex := range accents {
			hyprAccents[i] = hyprColor(hex)
		}
		activeBorder = bandedGradientLua(hyprAccents, 45)
	}
...
// bandedGradientBandRepeat controls how many times each color is repeated as
// a consecutive gradient stop in bandedGradientLua. Hyprland only supports
// evenly-spaced gradient stops -- there's no way to give a stop an explicit
// percentage position -- so a literal instant color cutoff isn't directly
// expressible. Repeating each color this many times packs the actual
// color-to-color blend into a single, thin stop gap (1 /
// (len(colors)*bandedGradientBandRepeat - 1) of the full border), while every
// other adjacent pair in a run is a same-color (zero-change) blend. That
// leaves each band occupying almost exactly its 1/len(colors) share, with a
// thin seam at each boundary instead of a wide blend across the whole run.
const bandedGradientBandRepeat = 10

// bandedGradientLua builds a Hyprland gradient that approximates hard-edged,
// equal-width bands for 2-5 colors. See bandedGradientBandRepeat for why this
// is an approximation rather than an exact cutoff.
func bandedGradientLua(colors []string, angle int) string {
	stops := make([]string, 0, len(colors)*bandedGradientBandRepeat)
